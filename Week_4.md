...
Deploying the server and implement foundational security controls
...
In the following screen-shots I replace the password with cryptographic keys. This is is generated with ssh-kegen -t ed255519. What is generated will be a pirvate key which will be kept on the workstation and a public key which will be sent to the server, ssh-copy-id t@192.168.56.101.
...
SSH is futher hardened by disabling direct root login and password-based authentication, enforing key based access only. This helps use reduce surface attacks and ensure secure remote administration. By disabling password login, root login we ensure security. The following show the changes made to sshd_config
